...
sharpen::RaftConsensus::RaftConsensus(
    ...
    // set log worker
    sharpen::IWorkerGroup *logWorker{new (std::nothrow)
                                         sharpen::SingleWorkerGroup{*this->scheduler_}};
    if (!logWorker) {
        throw std::bad_alloc{};
    }
    this->logWorker_.reset(logWorker);
    // load status cache
    this->LoadTerm();
    this->LoadVoteFor();
...
bool sharpen::RaftConsensus::DoPeersChangeable() const {
    return this->Writable() && this->changeable_.load() && !this->peersConfig_.Locked();
}

bool sharpen::RaftConsensus::PeersChangeable() const {
    assert(this->worker_ != nullptr);
    sharpen::AwaitableFuture<bool> future;
    this->worker_->Invoke(future,&Self::DoPeersChangeable,this);
    return future.Await();
}
...
sharpen::Mail sharpen::RaftConsensus::OnHeartbeatRequest(
    ...
    response.SetStatus(false);
    response.SetTerm(this->GetTerm());
    response.SetPeersEpoch(this->peersConfig_.GetEpoch());
        ...
        if (validatedEntries && lastIndex >= request.GetPreLogIndex() &&
            ...
            if (termOpt.Exist() && termOpt.Get() == request.GetPreLogTerm()) {
                ...
                if (lastIndex > request.GetPreLogIndex() + request.Entries().GetSize()) {
                    ...
                    this->logWorker_->InvokeUrgent(future,
                                                   &sharpen::ILogStorage::TruncateFrom,
                                                   this->logs_.get(),
                                                   request.GetPreLogIndex() + 1);
                    future.Await();
                    check = false;
                }
                ...
                sharpen::LogEntries entires{request.Entries(), offset};
                {
                    sharpen::AwaitableFuture<void> future;
                    this->logWorker_->InvokeUrgent(future,
                                                   &sharpen::ILogStorage::WriteBatch,
                                                   this->logs_.get(),
                                                   request.GetPreLogIndex() + 1 + offset,
                                                   std::move(entires));
                    future.Await();
                }
                response.SetStatus(true);
...
void sharpen::RaftConsensus::StepUp() {
    if (this->leaderCount_ != nullptr && !this->leaderCount_->TryStepUp(this->prevLeaderCount_)) {
        return;
    }
    this->role_ = sharpen::RaftRole::Leader;
    assert(this->heartbeatProvider_ != nullptr);
...
void sharpen::RaftConsensus::OnHeartbeatResponse(const sharpen::RaftHeartbeatResponse &response,
    ...
    if (response.GetStatus() && response.GetTerm() == this->GetTerm()) {
        ...
        // if the majority of peers finish change
        if (!this->changeable_ && response.GetPeersEpoch() == this->peersConfig_.GetEpoch()) {
            this->newPeersVotes_.emplace(actorId);
            if (this->newPeersVotes_.size() == this->peers_->GetMajority()) {
                this->changeable_ = true;
                if (!this->peersConfig_.Locked()) {
                    this->OnStatusChanged({sharpen::ConsensusResultEnum::PeersChangeable});
                }
            }
        }
    } else {
...
sharpen::ConsensusChangeResult sharpen::RaftConsensus::DoNviPreparePeersChanges() {
    assert(this->statusMap_ != nullptr);
    if (this->peersConfig_.Locked()) {
        return sharpen::ConsensusChangeResult::Confilct;
    }
    sharpen::ByteBuffer key{peersConfigKey};
    sharpen::ConsensusPeersConfiguration config{this->peersConfig_};
    config.Lock();
    sharpen::ByteBuffer val;
    sharpen::BinarySerializator::StoreTo(config, val);
    this->statusMap_->Write(std::move(key), std::move(val));
    this->peersConfig_ = std::move(config);
    return sharpen::ConsensusChangeResult::Success;
}

sharpen::ConsensusChangeResult sharpen::RaftConsensus::NviPreparePeersChanges() {
    assert(this->worker_ != nullptr);
    sharpen::AwaitableFuture<sharpen::ConsensusChangeResult> future;
    this->worker_->Invoke(future, &Self::DoNviPreparePeersChanges, this);
    return future.Await();
}

sharpen::ConsensusChangeResult sharpen::RaftConsensus::DoAbortPeersChanges() {
    assert(this->statusMap_ != nullptr);
    if (!this->peersConfig_.Locked()) {
        return sharpen::ConsensusChangeResult::Confilct;
    }
...
sharpen::ConsensusChangeResult sharpen::RaftConsensus::NviAbortPeersChanges() {
    assert(this->worker_ != nullptr);
    sharpen::AwaitableFuture<sharpen::ConsensusChangeResult> future;
    this->worker_->Invoke(future, &Self::DoAbortPeersChanges, this);
    return future.Await();
}

sharpen::ConsensusChangeResult sharpen::RaftConsensus::DoConfiguratePeers(
...
sharpen::ConsensusChangeResult sharpen::RaftConsensus::NviConfiguratePeers(
    std::function<std::unique_ptr<sharpen::IQuorum>(sharpen::IQuorum *)> configurater) {
    sharpen::AwaitableFuture<sharpen::ConsensusChangeResult> future;
...
void sharpen::RaftConsensus::NviDropLogsUntil(std::uint64_t index) {
    ...
    this->logWorker_->Submit(&sharpen::ILogStorage::DropUntil, this->logs_.get(), index);
}
...
sharpen::WriteLogsResult sharpen::RaftConsensus::NviWrite(const sharpen::LogBatch &logs) {
    ...
    this->logWorker_->Invoke(future, &Self::DoWrite, this, &logs);
    return future.Await();
}
...
sharpen::WriteLogsResult sharpen::RaftConsensus::DoWrite(const sharpen::LogBatch *logs) {
    ...
    assert(this->logAccesser_ != nullptr);
    // 1MB
    static constexpr std::size_t maxBatchBufSize{1024 * 1024};
    std::uint64_t lastIndex{this->GetLastIndex()};
    ...
    sharpen::LogEntries batch;
    batch.Reserve(logs->GetSize());
    std::size_t bufSize{0};
    std::size_t offset{0};
    for (std::size_t i = 0; i != logs->GetSize(); ++i) {
        sharpen::ByteBuffer entry{this->logAccesser_->CreateEntry(logs->Get(i), term)};
        bufSize += entry.GetSize();
        batch.Push(std::move(entry));
        if (bufSize > maxBatchBufSize) {
            sharpen::LogEntries tmp;
            std::swap(tmp, batch);
            std::size_t count{tmp.GetSize()};
            this->logs_->WriteBatch(beginIndex + offset, std::move(tmp));
            offset += count;
            if (offset != logs->GetSize()) {
                batch.Reserve(logs->GetSize() - offset);
            }
        }
    }
    if (!batch.Empty()) {
        this->logs_->WriteBatch(beginIndex + offset, std::move(batch));
    }
